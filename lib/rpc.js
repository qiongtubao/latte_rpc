 	...
 	function(require, exports, module, window) {
      ...
      (function() {
        this.reload = function(path){
          ...
          this.rpcRequire =  Modules.create(path+"/.latte");
          this.load(path);
          if(this.watcher) {
            this.watcher.close();
            this.watcher = null;
          ...
          if(this.config.watch) {
            this.watcher = latte_lib.fs.watch(this.config.path, function() {
              self.reload();
            });
          }
        }
        this.load = function(path) {
          var self = this;
  				...
  				files.forEach(function(filename) {
						var o;
						try {
								o = self.rpcRequire.require("./"+filename);
						}catch(err) {
								return self.emit("rpcLoadError", err);
						}
						if(o.handle) {
